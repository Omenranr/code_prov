...
class Scenario {
    ...
    addTableFromCSV(tableId, csv, category, config = {}) {      
        ...
        for (var i = 1; i < nlines; i++) {
                ...
                let elt = {}
                for (var c=0; c<ncols; c++) {
                    if(tableId=="inputmlfile") {console.log(tableId, vals)}
                    let val = vals[c]
    ...
    score(mlkey, statuscb, cb = undefined) {
        ...
        function doscore() {
            ...
            if ('fields' in config[mlkey])
                ...
                payload.fields = inputTable.cols[0].split(",")
                // payload.fields = ["m0", "m1", "m2"]

            // for (let r in inputTable.rows) {
            //         let data = []
            //         for (let c in payload.fields) {
            //             splitedRow = inputTable.rows[0][r].split(",")
            ...
            // }
            payload.values = [inputTable.rows[0][inputTable.cols[0]].split(",")]
            console.log("rows",inputTable.rows)
            statuscb('SCORING')
